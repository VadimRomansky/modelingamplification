...
void Simulation::evaluateCR(){
	...
	for(int j = 0; j < pgridNumber; ++j){
		...
		for(int i = 0; i < rgridNumber; ++i){
			...
			if(i == 0){
				f[i] = - lower[i]*distributionFunction[i][j]*p3;
				//f[i] = - lower[i]*distributionFunction[i][j];
			}

			f[i] = - distributionFunction[i][j]*p3;
			//f[i] = - distributionFunction[i][j];
			if(i == 0){
				f[i] = f[i] - upper[i]*distributionFunction[i+1][j]*p3 + (upper[i] + lower[i])*distributionFunction[i][j]*p3;
				//f[i] = f[i] - upper[i]*distributionFunction[i+1][j] + (upper[i] + lower[i])*distributionFunction[i][j];
			} else if (i == rgridNumber - 1){
				f[i] = f[i] - lower[i]*distributionFunction[i-1][j]*p3 + (upper[i] + lower[i])*distributionFunction[i][j]*p3;
				//f[i] = f[i] - lower[i]*distributionFunction[i-1][j] + (upper[i] + lower[i])*distributionFunction[i][j];
			} else {
				f[i] = f[i] - lower[i]*distributionFunction[i-1][j]*p3 - upper[i]*distributionFunction[i+1][j]*p3 + (upper[i] + lower[i])*distributionFunction[i][j]*p3;
			...
			if(volumeDerivative[i] > 0){
				if(j == pgridNumber - 1){
					...
					//double deltaP = pgrid[j+1] - pgrid[j];
					//derivative = (distributionFunction[i][j+1] - distributionFunction[i][j])/(3*deltaLogP);
					derivative = (cube(pgrid[j+1])*distributionFunction[i][j+1] - p3*distributionFunction[i][j])/(3*deltaLogP);
				}
			} else {
				if(j == 0){
					derivative = 0;
				} else {
					double deltaP = pgrid[j] - pgrid[j - 1];
					//derivative = (distributionFunction[i][j] - distributionFunction[i][j-1])/(3*deltaLogP);
					derivative = (p3*distributionFunction[i][j] - cube(pgrid[j-1])*distributionFunction[i][j-1])/(3*deltaLogP);
				}
			}

			f[i] += - volumeDerivative[i]*derivative*dtDivdr[i];

			if(i > 0){
				//f[i] += middleVelocity[i]*(distributionFunction[i][j] - distributionFunction[i-1][j])*dtDivdr[i];
				f[i] += p3*(distributionFunction[i][j]*middleVelocity[i] - distributionFunction[i-1][j]*middleVelocity[i-1])*dtDivdr[i];
			}
		...
		f[rgridNumber-1] -= upper[rgridNumber-1]*distributionFunction[rgridNumber-1][j]*p3;
	...
	for(int i = 0; i < rgridNumber; ++i){
		for(int j = 0; j < pgridNumber; ++j){
			distributionFunction[i][j] = tempDistributionFunction[i][j]/cube(pgrid[j]);
