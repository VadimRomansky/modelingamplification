...
void output(Simulation& simulation){
	...
	fclose(outProfile);*/
    SpaceBin* bin = simulation.bins[0][0];
	...
	bin = simulation.bins[simulation.shockWaveIndex][0];
	...
	bin = simulation.bins[simulation.xgridNumber-1][0];
...
void outputRPDF(std::list< Particle*>& list,const char* fileName){
	...
	while(it != list.end()){
		...
		if(maxp < abs(particle->localMomentumX)){
			maxp = abs(particle->localMomentumX);
		}
		++it;
	}
	double* distribution = new double[pgridNumber];
	...
	while(it != list.end()){
		...
		mass = particle->mass;
		++it;
		double p = particle->localMomentumX;
		if (abs(p) >= maxp){
			continue;
		}
		for(int i =0; i< pgridNumber; ++i){
...
void outputParticles(std::vector<Particle*>& particles, const char* fileName){
	...
	while(it != particles.end()){
		fprintf(outParticles ,"%lf %lf \n",(*it)->absoluteX, (*it)->absoluteMomentum*1000000000000000000000000.0);
		++it;
	}
	fclose(outParticles );
}
