...
#pragma omp parallel for private(k)
		for(k = 0; k < pgridNumber; ++k){
			...
			for(int i = 1; i < rgridNumber-1; ++i){
				...
				lower[i-1] = -(deltaT/(2*dV))*(diffusionCoef[i-1][k]/dxm);
				middle[i] = 1 + (deltaT/(2*dV))*(diffusionCoef[i-1][k]/dxp + diffusionCoef[i][k]/dxm);
				upper[i] = -(deltaT/(2*dV))*(diffusionCoef[i][k]/dxp);
				...
				if(i == shockWavePoint){
					...
					double v1 = middleVelocity[i-1] + vscattering[i-1];
					f[i] = distributionFunction[i][k]  + deltaT*((1/(2*dV))*(diffusionCoef[i][k]*(distributionFunction[i+1][k] - distributionFunction[i][k])/dxp - diffusionCoef[i-1][k]*(distributionFunction[i][k] - distributionFunction[i-1][k])/dxm) - (1/dV)*0.5*(v2*distributionFunction[i+1][k] - v1*distributionFunction[i-1][k]));
				}
				...
				if((xp*xp*middleVelocity[i+1] - xm*xm*middleVelocity[i]) < 0){
					...
					if(gkp - gkm < 0)
						...
						f[i] += -(deltaT/3)*((v2 - v1)/dV)*((gkm)/deltaLogP);
						middle[i] += -(deltaT/3)*((v2 - v1)/dV)/deltaLogP;
				} else {
					double v2 = middleVelocity[i+1] + vscattering[i+1];
					double v1 = middleVelocity[i] + vscattering[i];
					if(gkpp - gkp > 0)
			...
			lower[rgridNumber-2] = -(deltaT/(2*dV))*(diffusionCoef[rgridNumber-1][k]/dxp);
			middle[rgridNumber-1] = 1 + (deltaT/(2*dV))*(diffusionCoef[rgridNumber-1][k]/dxm);
			f[rgridNumber-1] = distributionFunction[rgridNumber-1][k] - (deltaT/(2*dV))*(diffusionCoef[rgridNumber-2][k]*(distributionFunction[rgridNumber-1][k] - distributionFunction[rgridNumber - 2][k])/dxm)
								- (deltaT/dV)*(middleVelocity[rgridNumber-1]*gip - middleVelocity[rgridNumber-2]*gim)
								 + (deltaT/3)*((middleVelocity[rgridNumber-1] - middleVelocity[rgridNumber-2])/dV)*((gkp - gkm)/deltaLogP);
