...
void Simulation::evaluateHydrodynamic() {
	...
	TracPen(tempDensity, dFlux);

	TracPen(tempMomentum, mFlux);
...
void Simulation::TracPen(double* u, double* flux){
...
void Simulation::updateTimeStep(){
	...
	for(int i = 1; i < rgridNumber; ++i){
        if(abs2(middleVelocity[i] - middleVelocity[i-1])*tempdt > abs2(3.0*deltaLogP*middleDeltaR[i])){
            tempdt = 0.5*abs2(3.0*deltaLogP*middleDeltaR[i]/(middleVelocity[i] - middleVelocity[i-1]));
		}
	}
	...
	for(int i = 1; i < rgridNumber; ++i){
		...
		double a = ((diffusionCoef[i-1][kgridNumber-1]/dxm + diffusionCoef[i][kgridNumber-1]/dxp) - (diffusionCoef[i-1][kgridNumber-1]/dxm) -(diffusionCoef[i][kgridNumber-1]/dxp))/(2*dx);
		if(1 + tempdt*a < 0){
            tempdt = 0.5/abs2(a);
		}
	}
	...
	for(int i = 1; i < rgridNumber-1; ++i){
		...
		for(int j = 1; j < pgridNumber; ++j){
			...
			double der = (1/(2*dV))*(diffusionCoef[i][j]*(distributionFunction[i+1][j] - distributionFunction[i][j])/dxp
							- diffusionCoef[i-1][j]*(distributionFunction[i][j] - distributionFunction[i-1][j])/dxm)
							- (1/dV)*(middleVelocity[i]*distributionFunction[i][j] - middleVelocity[i-1]*distributionFunction[i-1][j]);
