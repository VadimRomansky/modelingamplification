...
SpaceBin::SpaceBin(double X, double deltaX, double Y, double deltaY, double ux, double uy, double rho, double t, double b, int i, int j, bool scattering, int freeTimeEvaluation){
	x = X;
	x1 = x - deltaX/2;
	x2 = x + deltaX/2;
...
int* SpaceBin::propagateParticle(Particle* particle ,double& time, double timeStep, const int xgridNumber, const int ygridNumber){
	if(time != time){
		printf("aaa\n");
	}
	...
	double gammaFactor = 1/sqrt(1 - (Ux*Ux + Uy*Uy)/c2);
	...
	if(smallAngleScattering){
		...
		return binByCoordinates(particle->absoluteX, particle->absoluteY, x1 - numberX*(x2 - x1), x2 - x1, xgridNumber, y1 - numberY*(y2 - y1), y2 - y1, ygridNumber);
...
void SpaceBin::largeAngleScattering(Particle* particle, double& time, double timeStep){
	...
	double absoluteVX = particle->getAbsoluteVX();
	double absoluteVY = particle->getAbsoluteVY();
	...
	particle->absoluteX += absoluteVX*deltat/gammaFactor;
	particle->absoluteY += absoluteVY*deltat/gammaFactor;

	if(deltat < 0){
		printf("dt < 0\n");
		FILE* errFile = fopen("./output/errFile.dat","a");
		fprintf(errFile,"dt < 0\n");
		fprintf(errFile,"x= %lf, v= %lf, vx= %lf, x1= %lf, x2=%lf\n",particle->absoluteX, particle->getAbsoluteV(), absoluteVX,x1,x2);
		fclose(errFile);
	}
	...
	if(uniRandom() <= probability){
		...
		double sinLocalTheta  = sqrt(1 - cosLocalTheta*cosLocalTheta);

		particle->localMomentumX = particle->localMomentum*cosLocalTheta;
		particle->localMomentumY = particle->localMomentum*sinLocalTheta*cosLocalPhi;

		particle->setAbsoluteMomentum(Ux, Uy);
...
int* SpaceBin::binByCoordinates(double x, double y, double x0, double deltax, const int xgridNumber, double y0, double deltay, const int ygridNumber){
	int i = lowerInt((x - x0)/deltax);
	int j = lowerInt((y - y0)/deltay);
...
void SpaceBin::makeOneStep(Particle* particle, double colisionTime, double& time){
	...
	double localMomentumY = particle->localMomentumY;

	double x = particle->absoluteX;
	double y = particle->absoluteY;


	double ux = particle->getAbsoluteVX();
	double uy = particle->getAbsoluteVY();
	...
	double deltaX = tempx - x;
	double deltaY = tempy - y;


	while( (abs(deltaX) > (x2 - x1)/10) || (abs(deltaY) > (y2 - y1)/10)){
		deltat = deltat/10;
...
void SpaceBin::scattering(Particle* particle, double maxTheta){
	...
	double localMomentumZ = sqrt(particle->localMomentum*particle->localMomentum - particle->localMomentumX*particle->localMomentumX - particle->localMomentumY*particle->localMomentumY);
	double localPhi = atan2(localMomentumZ, particle->localMomentumX);
	...
	double cosPrevTheta = cos(localTheta);
	double sinPrevTheta = sin(localTheta);
	...
	double cosDPhi = (sinPrevTheta*cosDeltaTheta - cosPrevTheta*sin(deltaTheta)*cos(deltaPhi))/sinLocalTheta;
	double dPhi = acos(cosDPhi);
	if(deltaPhi > pi){
		deltaPhi = -deltaPhi;
	}

	localPhi = localPhi + dPhi;
	
	particle->localMomentumX = particle->localMomentum*cos(localTheta);
	particle->localMomentumY = particle->localMomentum*sinLocalTheta*cos(localPhi);
...
double SpaceBin::evaluateCrymskyIntegralX(){
	...
	while(it != particles.end()){
		...
		++it;
		particle->setLocalMomentum(Ux, Uy);
		integral += particle->localMomentumX*particle->weight/getFreeTime(particle);
	}
...
double SpaceBin::evaluateCrymskyIntegralY(){
	...
	while(it != particles.end()){
		...
		++it;
		particle->setLocalMomentum(Ux, Uy);
		integral += particle->localMomentumY*particle->weight/getFreeTime(particle);
	}
