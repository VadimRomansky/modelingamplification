...
void Simulation::solveUpstream1(){
	...
	for(int i = 0; i < rgridNumber; ++i){
		double H = upstreamBins1[i]->density*qube(oldReverseShockWaveR/reverseShockWaveR) - density[i-1]*(3*tau*upstreamBins1[i+1]->xi*upstreamBins1[i+1]->xi*(velocity[i+1] - forwardV*upstreamBins1[i+1]->xi)/(qube(upstreamBins1[i+1]->xi) - qube(upstreamBins1[i]->xi)));
		velocity[i] = (upstreamBins1[i]->density*upstreamBins1[i]->U*exp(-3*tau) - density[i+1]*velocity[i+1]*3*tau*upstreamBins1[i+1]->xi*upstreamBins1[i+1]->xi/(forwardV*(qube(upstreamBins1[i+1]->xi) - qube(upstreamBins1[i]->xi)))-(pressure[i+1] - upstreamBins1[i+1]->pressure)*tau/(forwardV*(upstreamBins1[i+1]->xi - upstreamBins1[i]->xi)))/H;
		density[i] = H/(1 - 3*tau*upstreamBins1[i+1]->xi*upstreamBins1[i+1]->xi*(velocity[i] - forwardV*upstreamBins1[i]->xi)/(forwardV*(qube(upstreamBins1[i+1]->xi) - qube(upstreamBins1[i]->xi))));
		pressure[i] = (upstreamBins1[i]->pressure*exp(-3*tau) - pressure[i+1]*3*tau*sqr(upstreamBins1[i+1]->xi)*(velocity[i+1] - forwardV*upstreamBins1[i+1]->xi)/(forwardV*(qube(upstreamBins1[i+1]->xi) - qube(upstreamBins1[i]->xi))))*
			(1 - 3*tau*(sqr(upstreamBins1[i]->xi)*(velocity[i] - forwardV*upstreamBins1[i]->xi) + (gamma - 1)*(sqr(upstreamBins1[i+1]->xi)*upstreamBins1[i+1]->U - sqr(upstreamBins1[i]->xi)*upstreamBins1[i]->U))/(forwardV*(qube(upstreamBins1[i+1]->xi) - qube(upstreamBins1[i]->xi))));
	}
